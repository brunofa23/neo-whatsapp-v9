...
export default class DatasourcesController {
  ...
  async scheduledPatients() {
    ...
    //     const pacQuery = `SELECT top 1 1 interaction, '123', pac_nome, '31985228619' AS pac_celular, pac_ind_whatsapp, agm_hini,
    //     CONCAT(
    //       RIGHT('0' + CAST(DAY(AGM_HINI) AS VARCHAR(2)), 2), '/',
    //       RIGHT('0' + CAST(MONTH(AGM_HINI) AS VARCHAR(2)), 2), '/',
    //       CAST(YEAR(AGM_HINI) AS VARCHAR(4))
    //     ) AS data_agm,
    //   CONCAT('Olá ABEL tudo bem? temos uma consulta agendada para o dia', FORMAT(CONVERT(datetime,agm_hini), 'dd/MM/yyyy HH:mm')+' gostaria de confirmar? 1-Sim 2-Não') as message
    //   FROM (
    // SELECT pac_reg, pac_nome, '3185228619' AS pac_celular, pac_ind_whatsapp, agm_hini,
    //       ROW_NUMBER() OVER (PARTITION BY pac_nome ORDER BY agm_hini) AS row_num
    // FROM pac
    ...
    // SELECT top 1 1 interaction, '456', 'PACIENTE 2', '3190691174' AS pac_celular, pac_ind_whatsapp, agm_hini,
    //     CONCAT(
    //       RIGHT('0' + CAST(DAY(AGM_HINI) AS VARCHAR(2)), 2), '/',
    //       RIGHT('0' + CAST(MONTH(AGM_HINI) AS VARCHAR(2)), 2), '/',
    //       CAST(YEAR(AGM_HINI) AS VARCHAR(4))
    //     ) AS data_agm,
    //   CONCAT('Olá PACIENTE 2 tudo bem? temos uma consulta agendada para o dia', FORMAT(CONVERT(datetime,agm_hini), 'dd/MM/yyyy HH:mm')+' gostaria de confirmar? 1-Sim 2-Não') as message
    //   FROM (
    // SELECT pac_reg, pac_nome, '3185228619' AS pac_celular, pac_ind_whatsapp, agm_hini,
    //       ROW_NUMBER() OVER (PARTITION BY pac_nome ORDER BY agm_hini) AS row_num
    // FROM pac
