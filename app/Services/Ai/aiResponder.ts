...
export async function responderPergunta(perguntaUsuario: string): Promise<string> {
  const file = fs.readFileSync(path, 'utf-8')
  const base = JSON.parse(file)

  const perguntas = base.map((p: any) => p.pergunta)
  ...
  if (match.bestMatch.rating > 0.75) {
    const respostaBase = base.find((p: any) => p.pergunta === match.bestMatch.target).resposta
    return respostaBase
  }
  ...
  const contexto = base.map((p: any) => `Q: ${p.pergunta}\nA: ${p.resposta}`).join('\n\n')
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      ...
      { role: 'user', content: `Baseado nas perguntas abaixo responda de forma direta:
        ${contexto} INFORMAÇÕES DO PACIENTE:${perguntaContexto} Pergunta: ${perguntaUsuario}` },
    ],
    temperature: 0.5,
  })
