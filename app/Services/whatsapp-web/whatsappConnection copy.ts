...
//FUNÇÕES PARA CHAMAR O SENDMESSAGE
const sendTimers = new Map<number, NodeJS.Timeout>()
const sendingLocks = new Set<number>()

function stopSendLoop(agentId: number) {
  const t = sendTimers.get(agentId)
  if (t) clearTimeout(t)
  sendTimers.delete(agentId)
  sendingLocks.delete(agentId)
}

async function startSendLoop(client: any, agent: any) {
  // garante que não duplica loop em reconnect
  stopSendLoop(agent.id)

  const tick = async () => {
    try {
      // evita overlap (um envio por vez por agent)
      if (sendingLocks.has(agent.id)) return
      sendingLocks.add(agent.id)
      ...
      console.log("Erro no loop de envio:", e)
    } finally {
      sendingLocks.delete(agent.id)
      ...
      sendTimers.set(agent.id, timeoutId)
    }
  }
