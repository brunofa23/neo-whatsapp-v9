...
const whatsappClients: { [key: string]: Client } = {};
...
const whatsappConnections = new Map(); // Armazena os clientes por ID
...
async function startAgent(_agent: Agent) {
  ...
  client.on('ready', async () => {
    ...
    whatsappClients[_agent.id] = client; // Armazena o cliente
  ...
  whatsappConnections.set(_agent.id, client);
  console.log(">>>>>", whatsappConnections)
  return client
}


function getWhatsAppClient(agentId) {
  // return whatsappConnections.get(agentId) || null; // Retorna o cliente ou null se não encontrado
  return whatsappClients[agentId] || null;
}

async function destroyAgent(agentId) {
  const client = whatsappConnections.get(agentId);
  if (client) {
    await client.destroy(); // Fecha a conexão
    whatsappConnections.delete(agentId); // Remove do Map
    console.log(`Cliente ${agentId} desconectado e removido.`);
  } else {
    console.log(`Cliente ${agentId} não encontrado.`);
  }
}
