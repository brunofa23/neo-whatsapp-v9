...
function getClassifierThreshold() {
    const threshold = Number(Env_1.default.get('WAITING_TIME_INTENT_THRESHOLD', 0.8));
...
async function getRecentTalkContext(fromKey, fromDigits, chatnumber) {
    try {
        const query = Talk_1.default.query()
        ...
        return rows
            ...
            created_at: talk.createdAt?.toISO?.() || null,
        }));
    }
    catch (error) {
        await Log_1.default.create({
...
async function classifyWaitingTimeIntent(params) {
    ...
    const messages = [
        {
            role: 'system',
            content: 'Você é um classificador de intenção para mensagens de pacientes de uma clínica oftalmológica. ' +
...
${JSON.stringify(payload, null, 2)}`,
    ...
    try {
        const response = await axios_1.default.post('https://api.groq.com/openai/v1/chat/completions', {
            model: String(Env_1.default.get('WAITING_TIME_INTENT_MODEL', WAITING_TIME_CLASSIFIER_DEFAULT_MODEL)),
            messages,
            temperature: 0,
            max_tokens: 180,
        }, {
        ...
        if (!parsed)
            throw new Error(`JSON inválido do classificador: ${content}`);
        const confidence = Number(parsed.confidence || 0);
        const shouldAutoReply = parsed.shouldAutoReply === true &&
