...
async function startSendLoop(client, agent) {
    stopSendLoop(agent.id);
    const tick = async () => {
        try {
            if (sendingLocks.has(agent.id))
                return;
            ...
            const delay = await (0, util_1.GenerateRandomTime)(startTimeSendMessage, endTimeSendMessage, '----Time Send Message');
