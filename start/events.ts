...
const LOG_CLEANUP_COOLDOWN_MS = 6 * 60 * 60 * 1000; // 6 horas

async function cleanupLogs15Days(force = false) {
  try {
    const now = Date.now();
    if (!force && now - lastLogCleanupAt < LOG_CLEANUP_COOLDOWN_MS) return;
...
async function sendRepeatedMessage() {
  ...
  const tick = async () => {
    if (running) {
      console.log(
        "[sendRepeatedMessage] tick ignorado (execução anterior ainda em andamento)"
      );
      scheduleNext();
      return;
    }
    ...
    try {
      // ✅ limpa logs antigos (mantém 15 dias)
      ...
      if (await TimeSchedule()) {
        ...
        for (const date of targetDates) {
          ...
          console.log(
            `Buscando dados no Smart(Server) [interaction=1]: ${formatted}`
          );
...
async function sendRepeatedMessageKlingo() {
  ...
  const computeTargetDate = () => {
    ...
    const daysToAdd =
      today.weekday >= 1 && today.weekday <= 4
        ? 2 // seg-qui → +2
        : today.weekday === 5
        ? 3 // sex → +3 (segunda)
        : today.weekday === 6
  ...
  const schedulesTick = async () => {
    ...
    if (schedulesRunning) {
      console.log(
        "[Klingo][getSchedules] tick ignorado (execução anterior em andamento)"
      );
      return;
    }
  ...
  const confirmTick = async () => {
    ...
    if (confirmRunning) {
      console.log(
        "[Klingo][confirmOrCancel] tick ignorado (execução anterior em andamento)"
      );
      return;
    }
    ...
    try {
      ...
      if (await TimeSchedule()) {
        console.log(
          `Atualizando confirmações no Klingo: ${DateTime.now()
            .setZone(zone)
...
async function resendMessage() {
  ...
  const tick = async () => {
    ...
    if (running) {
      console.log(
        "[resendMessage] tick ignorado (execução anterior ainda em andamento)"
      );
      return;
    }
    ...
    try {
      ...
      const updatedResendResult = await Shippingcampaign.query()
        ...
        .where(
          "dateshedule",
          ">=",
          tomorrowStart.toSQL({ includeOffset: false })
        )
        .where(
          "dateshedule",
          "<=",
          tomorrowEnd.toSQL({ includeOffset: false })
        )
